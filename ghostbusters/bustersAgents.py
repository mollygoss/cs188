...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        closestMove = float("inf")
        best_action = None
        ghostPos = None

        #find the closest "best guess" ghost position
        for distribution in livingGhostPositionDistributions:
            maze_distance = self.distancer.getDistance(pacmanPosition, distribution.argMax())
            if maze_distance < closestGhost:
                closestGhost = maze_distance
                ghostPos = distribution.argMax()


        #now discover which action is best for getting us near the ghost
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPosition, ghostPos)
            if distance < closestMove:
                best_action = action
                closestMove = distance
